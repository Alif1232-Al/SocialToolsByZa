export type FrameText = {
  ...
  color: string;
  bgColor: string;
  position: "top-bar" | "bottom-bar" | "top-corner" | "bottom-corner";
  fontSize: number;
};
...
export type CustomFrameOptions = {
  ...
  textColor: string;
  textBg: string;
  textPosition: "top-bar" | "bottom-bar";
...
export function drawFrameText(
  ...
  const fontSize = ft.fontSize || Math.round(barH * 0.5);
  let barY: number;

  if (ft.position === "top-bar") {
    barY = 0;
  } else if (ft.position === "bottom-bar") {
    barY = h - barH;
  } else {
    return;
  }

  ctx.fillStyle = ft.bgColor;
  ctx.fillRect(0, barY, w, barH);
  ctx.fillStyle = ft.color;
  ctx.font = `bold ${fontSize}px "Inter","Arial",sans-serif`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText(ft.text.toUpperCase(), w / 2, barY + barH / 2);
}

export function drawCustomFrameText(
  ctx: CanvasRenderingContext2D,
  w: number, h: number,
  text: string, textColor: string, textBg: string,
  position: "top-bar" | "bottom-bar"
) {
  const barH = Math.round(h * 0.09);
  const fontSize = Math.round(barH * 0.5);
  const barY = position === "top-bar" ? 0 : h - barH;
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText(text.toUpperCase(), w / 2, barY + barH / 2);
}
...
export const FRAMES: FrameDef[] = [
  {
    ...
    render(ctx, w, h) {
      ...
      for (let i = 0; i < 30; i++) {
        ctx.beginPath();
        ctx.arc(rng() * w, rng() * h, 2 + rng() * 3, 0, Math.PI * 2);
        ctx.fill();
      }
    },
    ...
    text: { text: "BOOM!", color: "#fff", bgColor: "#111", position: "bottom-bar", fontSize: 22 },
  },
  {
    id: "blue-ocean",
    name: "Blue Ocean",
    ...
    render(ctx, w, h) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        ctx.beginPath();
        for (let x = 0; x < w; x += 4) {
          const wy = y + Math.sin((x + i * 50) * 0.02) * 6 + Math.sin((x + i * 30) * 0.04) * 3;
    ...
    showNumbers: true, watermark: true,
    text: { text: "CHILL VIBES", color: "#fff", bgColor: "#1e3a5f", position: "bottom-bar", fontSize: 20 },
  },
  {
    id: "snowy",
    name: "Snowy",
    ...
    render(ctx, w, h) {
      ...
      ctx.lineWidth = 1;
      for (let i = 0; i < 8; i++) {
        const sx = rng() * w, sy = rng() * h, sz = 3 + rng() * 5;
        ctx.beginPath();
        for (let j = 0; j < 6; j++) {
          const a = (Math.PI / 3) * j;
          ctx.moveTo(sx, sy);
    ...
    showNumbers: true, watermark: true,
    text: { text: "WINTER", color: "#fff", bgColor: "#0c4a6e", position: "bottom-bar", fontSize: 22 },
  },
  {
    id: "midnight",
    name: "Midnight",
    ...
    text: { text: "STARLIGHT", color: "#fbbf24", bgColor: "rgba(0,0,0,0.6)", position: "bottom-bar", fontSize: 20 },
  },
  {
    id: "pastel-dream",
    name: "Pastel Dream",
    ...
    showNumbers: true, watermark: true,
    text: { text: "DREAMY", color: "#fff", bgColor: "#8b5cf6", position: "top-bar", fontSize: 22 },
  },
  {
    id: "neon-cyber",
    name: "Neon Cyber",
    ...
    text: { text: "CYBER", color: "#06b6d4", bgColor: "#111", position: "bottom-bar", fontSize: 24 },
  },
  {
    id: "vintage-sepia",
    name: "Vintage Sepia",
    ...
    text: { text: "TIMELESS", color: "#92400e", bgColor: "rgba(255,255,255,0.5)", position: "bottom-bar", fontSize: 20 },
  },
  {
    id: "tropical",
    name: "Tropical",
    ...
    showNumbers: true, watermark: true,
    text: { text: "PARADISE", color: "#fff", bgColor: "#065f46", position: "bottom-bar", fontSize: 20 },
  },
  {
    id: "bubblegum",
    name: "Bubblegum",
    ...
    showNumbers: true, watermark: true,
    text: { text: "SWEET", color: "#fff", bgColor: "#9d174d", position: "top-bar", fontSize: 22 },
  },
  {
    id: "monochrome",
    name: "Monochrome",
    ...
    showNumbers: false, watermark: true,
    text: { text: "CLASSIC", color: "#fff", bgColor: "#000", position: "bottom-bar", fontSize: 22 },
  },
  {
    id: "sunset",
    name: "Sunset",
    ...
    showNumbers: false, watermark: false,
    text: { text: "GOLDEN HOUR", color: "#fff", bgColor: "#2e1065", position: "bottom-bar", fontSize: 18 },
  },
  {
    id: "forest",
    name: "Forest",
    ...
    showNumbers: false, watermark: true,
    text: { text: "NATURE", color: "#fff", bgColor: "#064e3b", position: "top-bar", fontSize: 22 },
  },
];
